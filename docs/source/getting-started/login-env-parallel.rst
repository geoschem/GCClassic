...
.. _env-files-envvars-parallel-envvars:

=================================================
Environment variables controlling parallelization
=================================================

.. _env-files-envvars-parallel-threads:
...
We recommend that you define :envvar:`OMP_NUM_THREADS`
not only in your environment file, but also in your :ref:`GEOS-Chem
run script <run-script>`.

.. _env-files-envvars-parallel-stack:
...
Be on the lookout for these errors:
   ...
   #. If :ref:`env-files-envvars-parallel-threads` is set to 1, then
      your simulation will execute using only one computational
      core. This will make your simulation take much longer than
      necessary. |br|
      |br|

   #. If the :ref:`env-files-envvars-parallel-stack` environment
      variable is not included in your environment file (or if it is
      set to a very low value), you might encounter a
      :ref:`errguide-segfault-tpcore` error.  In this case, GEOS-Chem
      Classic "thinks" that it does not have enough memory to perform
      the simulation, even though sufficient memory may be present.
